...
resource "azurerm_public_ip" "pip" {
  name                = "${var.pip_name}-${terraform.workspace}-${var.n3}"
...
resource "azurerm_network_security_group" "nsg_cka" {
  name                = "${var.nsg_name}-${terraform.workspace}-${var.n3}"
  location            = var.rg_region
  resource_group_name = var.rg_name
...
resource "azurerm_network_interface" "nic_cka" {
  name                = "${var.nic_ip_name}-${terraform.workspace}-${var.n3}"
  location            = var.rg_region
  resource_group_name = var.rg_name

  ip_configuration {
    name                          = "${var.nic_name}-${terraform.workspace}-${var.n3}"
...
resource "azurerm_linux_virtual_machine" "vm_cka" {
  name                  = "${var.vm_name}-${terraform.workspace}-${var.n3}"
  location              = var.rg_region
  resource_group_name   = var.rg_name
...
os_disk {
    name                 = "${var.myOsDisk}-${terraform.workspace}-${var.n3}"
    caching              = "ReadWrite"
...
source_image_reference {
  ...
  computer_name  = "${var.vm_name}-${terraform.workspace}-${var.n3}"
